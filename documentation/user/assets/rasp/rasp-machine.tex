...
\begin{tikzpicture}[node distance=0.7cm, inner sep=0pt]% Example:
    ...
    % input tape
    \node[hblock] (input) [above= 3cm of rasp] {};
    ...
    % output tape    
    \node[hblock] (output) [below= 3cm of rasp] {};
    ...
    % arrows
    \draw[head] (rasp.west) .. controls +(-2,0.5) and +(3,-0.5) .. (pi.east);
    \draw[head] (rasp.north) .. controls +(0.5,2) and +(-0.5,-3) .. (ii.south);
    \draw[head] (rasp.south) .. controls +(0.5,-2) and +(-0.5,3) .. (oi.north);
